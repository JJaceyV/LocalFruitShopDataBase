...
set dateformat DMY;
--INSERT THÔNG TIN VÀO BẢNG LOAIHANG
...
--INSERT THÔNG TIN VÀO BẢNG GIAMGIA
...
--INSERT THÔNG TIN VÀO BẢNG MATHANG
...
--INSERT THÔNG TIN VÀO BẢNG NHACUNGCAP
...
--INSERT THÔNG TIN VÀO BẢNG KHACHHANG
...
--INSERT THÔNG TIN VÀO BẢNG DONHANG
...
--INSERT THÔNG TIN VÀO BẢNG PHIEUNHAP
...
--INSERT THÔNG TIN VÀO BẢNG CHITIETPHIEUNHAP
...
--INSERT THÔNG TIN VÀO BẢNG PHIEUXUAT
...
--INSERT THÔNG TIN VÀO BẢNG CHITIETPHIEUXUAT
...
--INSERT THÔNG TIN VÀO BẢNG PHIEUGIAOHANG
...
--INSERT THÔNG TIN VÀO BẢNG THANHTOAN
...
--INSERT THÔNG TIN VÀO BẢNG PHANQUYEN
...
--INSERT THÔNG TIN VÀO BẢNG TAIKHOAN_QUYEN
...
--INSERT THÔNG TIN VÀO BẢNG KHACHHANG_QUYEN
...
--CHO BIẾT HOTEN, DIACHI, SDT CỦA NHÂN VIÊN NỮ
SELECT HoTen, DiaChi, Sdt FROM NHANVIEN
WHERE GioiTinh = N'Nữ'

--CHO BIẾT HOTEN KHACH HANG, SDT CỦA NHỮNG KHÁCH HÀNG CÓ ĐỊA CHỈ Ở TPHCM 
SELECT HoTen, Sdt, DiaChi FROM KHACHHANG 
WHERE DiaChi LIKE N'%TP.HCM%'

--CHO BIẾT CÁC MẶT HÀNG CÓ GIÁ TRÊN BIET 500000
SELECT * FROM MATHANG
WHERE GiaBan > 500000

--CHO BIẾT MA NHAN VIEN, TENNV, SDT CUA NHANVIEN CÓ CHỨC VỤ LÀ BÁN HÀNG 
SELECT MaNV, HoTen, Sdt
FROM NHANVIEN WHERE ChucVu = N'Bán Hàng'

--LIỆT KÊ HỌ TÊN NHÂN VIÊN VÀ SỐ HÓA ĐƠN THANH TOÁN DO NHÂN VIÊN ĐÓ ĐẢM NHẬN 
SELECT NHANVIEN.HoTen, MaHDTT
FROM NHANVIEN INNER JOIN THANHTOAN ON NHANVIEN.MaNV = THANHTOAN.MaNV

--CHO BIẾT TÊN KHÁCH HÀNG, TÊN MẶT HÀNG, SỐ LƯỢNG MẶT HÀNG MÀ KHÁCH MUA > 5  
...
--CHO BIẾT NHỮNG ĐƠN HÀNG CHƯA THANH TOÁN LÀ CỦA KHÁCH HÀNG NÀO 
...
--LIỆT KÊ MÃ PX, NGÀY XUẤT, MÃ KH CỦA CÁC PHIẾU XUẤT PHÁT SINH TRONG QUÝ 2/2022
...
---AND
----HIỂN THỊ DANH SÁCH NHÂN VIÊN CÓ THÔNG TIN BAO GỒM MÃ NV, HỌ TÊN, CHỨC VỤ, ĐỊA CHỈ, GIỚI TÍNH LÀ NỮ VÀ TRONG TÊN CÓ CHỮ 'LAN' 
SELECT MaNV, HoTen, GioiTinh,ChucVu,DiaChi
FROM NHANVIEN
WHERE GioiTinh = N'Nữ' AND HoTen Like N'%Lan%'

---OR
---- HIỂN THỊ DANH SÁCH CÁC MẶT HÀNG BAO GỒM MÃ HÀNG, TÊN HÀNG, XUẤT XỨ, GIÁ BÁN, XUẤT XỨ TẠI HÀN QUỐC HOẶC VIỆT NAM
...
---LIKE
---- HIỂN THỊ DANH SÁCH KHÁCH HÀNG SỐNG TẠI TPHCM BAO GỒM THÔNG TIN VỀ MÃ KH, HỌ TÊN, ĐỊA CHỈ
SELECT MaKH, HoTen, DiaChi
FROM KHACHHANG
WHERE DiaChi LIKE '%TP.HCM%'

---BETWEEN
---- HIỂN THỊ DANH SÁCH ĐƠN HÀNG BAO GỒM THÔNG TIN VỀ MÃ ĐƠN HÀNG, MÃ KHÁCH HÀNG, NGÀY ĐẶT TRONG KHOẢNG TỪ 1/1/2022 - 16/3/2022
SELECT MaDH, MaKH, NgayDat
FROM DONHANG
WHERE NgayDat BETWEEN '1/1/2022' AND '16/03/2022'

---TÍNH THÀNH TIỀN CỦA CÁC ĐƠN ĐẶT HÀNG KHI CHƯA ÁP DỤNG KHUYỄN MÃI
---THÔNG TIN HIỂN THỊ BAO GỒM MÃ ĐƠN HÀNG, TỔNG TIỀN
...
(1-gg.MucGiam)*ctdh.GiaBan*ctdh.SoLuong as 'So tien sau khi giam', ctdh.GiaBan*ctdh.SoLuong - (1-gg.MucGiam)*ctdh.GiaBan*ctdh.SoLuong as 'So tien duoc giam'
...
select ctdh.MaDH,ctdh.MaHang,mh.TenHang, ctdh.SoLuong, ctdh.GiaBan, ctpn.DonGia, (ctdh.GiaBan - ctpn.DonGia) as 'Tienloi'
...
select cast(day(NgayGiao) as nvarchar(2)) + ' -' + cast(month(NgayGiao) as nvarchar(2)) 
		+ ' - ' + cast(year(NgayGiao) as nvarchar(4)) as Ngay ,count(NgayGiao) as soluongdongiao, nv.HoTen as nhanviengiaohang
...
---LIỆT KÊ TẤT CẢ CÁC QUYỀN VỪA CỦA NHÂN VIÊN, VỪACỦ A QUẢN LÝ
...
select MaQuyen,TenQuyen, LoaiQuyen
from PHANQUYEN
where MaQuyen in (select MaQuyen from TAIKHOAN_QUYEN except select MaQuyen from NHANVIEN nv, TAIKHOAN_QUYEN tkq where nv.MaNV = tkq.MaNV and nv.MaNV = 'NV001'
INTERSECT select MaQuyen from NHANVIEN nv, TAIKHOAN_QUYEN tkq where nv.MaNV = tkq.MaNV and nv.MaNV = 'NV002'
INTERSECT select MaQuyen from NHANVIEN nv, TAIKHOAN_QUYEN tkq where nv.MaNV = tkq.MaNV and nv.MaNV = 'NV003'
INTERSECT select MaQuyen from NHANVIEN nv, TAIKHOAN_QUYEN tkq where nv.MaNV = tkq.MaNV and nv.MaNV = 'NV004'
INTERSECT select MaQuyen from NHANVIEN nv, TAIKHOAN_QUYEN tkq where nv.MaNV = tkq.MaNV and nv.MaNV = 'NV005'
...
--CHO BIẾT KHÁCH HÀNG CÓ GIỚI TÍNH LÀ NỮ VÀ CÓ ĐỊA CHỈ GIAO Ở HÀ NỘI 
...
PIVOT ( COUNT(bangnguon.Maloai) for bangnguon.TenLoai in ( [Táo], [Nho], [Cam], [Bưởi], [Dưa hấu], [Việt quất], [Dâu tây], [Lê], [Cherry], [Xoài])) as bangchuyen

---LẬP BÁO CÁO SỐ PHIẾU NHẬP LẬP BỞI MỖI NHÂN VIÊN, PHÂN LOẠI THEO NHÀ CUNG CẤP
select MaNCC,TenNCC,[NV004], [NV005]
from (select MaNV, ncc.TenNCC, pn.MaNCC FROM PHIEUNHAP pn, NHACUNGCAP ncc where ncc.MaNCC = pn.MaNCC) as bangnguon
pivot 
...
---THỐNG KÊ TỔNG SỐ LƯỢNG CỦA MỖI MẶT HÀNG ĐƯỢC ĐẶT TRONG THÁNG 1, 2, 3, 4
...
create function dbo.demsoluongdathang (@MaKH nvarchar(5))
returns int
as
begin
return
(select bang.soluong
from (select dh.MaKH , count(dh.MaKH) AS soluong 
...
print dbo.demsoluongdathang('KH003')
print dbo.demsoluongdathang('KH026')
...
create user US04 for login lthanhnhaqlkho
...
exec SaoLuuDuLieu 'bantraicay', 'c:\backup\bantraicay.bak'
...
exec restoreDatabase 'fruit4','c:\backup:\bantraicay.bak'
